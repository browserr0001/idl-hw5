...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.criterion = nn.CrossEntropyLoss(ignore_index=self.tokenizer.pad_id,label_smoothing=self.config.get('training', {}).get('label_smoothing', 0.0))
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        # Unpack batch and move to device
          ...
          with torch.autocast(device_type=self.device, dtype=torch.float16):
              ...
              # Calculate loss - reshaping for CrossEntropyLoss
              # (batch_size, seq_len, vocab_size) -> (batch_size*seq_len, vocab_size)
              raw_loss = self.criterion(raw_preds.reshape(-1, raw_preds.size(-1)), 
       ...
       # raise NotImplementedError # Remove once implemented
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                seqs, scores = generator.generate_beam(prompts,beam_width=generation_config.get('beam_width', 5),temperature=generation_config.get('temperature', 1.0),repeat_penalty=generation_config.get('repeat_penalty', 1.0))
