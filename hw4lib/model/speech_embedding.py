...
class StackedBLSTMEmbedding(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, 
        ...
        super(StackedBLSTMEmbedding, self).__init__()
        self.stride1, self.stride2 = self.closest_factors(time_reduction)
        self.pool1_params = {"kernel_size": self.stride1, "stride": self.stride1, "padding": 0, "dilation": 1}
        self.pool2_params = {"kernel_size": self.stride2, "stride": self.stride2, "padding": 0, "dilation": 1}
        self.blstm1 = nn.LSTM(input_dim, hidden_dim // 2, num_layers=1, batch_first=True, bidirectional=True)
        self.blstm2 = nn.LSTM(hidden_dim, hidden_dim // 2, num_layers=1, batch_first=True, bidirectional=True)
        self.pool1 = nn.MaxPool1d(**self.pool1_params)
        self.pool2 = nn.MaxPool1d(**self.pool2_params)
        self.linear_embed = nn.Linear(hidden_dim, output_dim)
        self.dropout = nn.Dropout(dropout)
    ...
    def calculate_pool_output_length(self, L_in: torch.Tensor, pool_params: dict) -> torch.Tensor:
        numerator = (L_in + 2 * pool_params["padding"] - pool_params["dilation"] * (pool_params["kernel_size"] - 1) - 1)
        return (numerator // pool_params["stride"] + 1).to(torch.long)
...
class Conv2DSubsampling(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, dropout: float = 0.0, 
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, output_dim, kernel_size=3, stride=(self.time_stride1, 1)),
            nn.GELU(),
            nn.Conv2d(output_dim, output_dim, kernel_size=3, stride=(self.time_stride2, 1)),
            nn.GELU(),
        )
        self.freq_pool = nn.AdaptiveAvgPool2d((None, 1))
        self.linear_out = nn.Linear(output_dim, output_dim)  # FIXED: match freq pooled channel dim
        self.dropout = nn.Dropout(dropout)
...
class SpeechEmbedding(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, time_reduction: int = 6, 
        ...
        if reduction_method in ['conv', 'both']:
            self.cnn = Conv2DSubsampling(input_dim, self.embedding_dim, dropout=dropout, time_reduction=conv_reduction)
        if reduction_method in ['lstm', 'both']:
            lstm_input_dim = self.embedding_dim if self.cnn else input_dim
            self.blstm = StackedBLSTMEmbedding(lstm_input_dim, self.embedding_dim, self.embedding_dim, time_reduction=lstm_reduction, dropout=dropout)
